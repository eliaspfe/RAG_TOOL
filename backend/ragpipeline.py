    ...
    def _fetch_model_info(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(
                    f"[{datetime.now()}] Local LLM-Service verbunden: {self.model_name}"
                )
                return
            except Exception as e:
                if attempt < max_retries - 1:
                    print(
                        f"[{datetime.now()}] WARNUNG: Kann Local LLM Model-Info nicht abrufen (Versuch {attempt+1}/{max_retries}): {e}"
                    )
                    time.sleep(retry_delay)
                else:
                    print(
                        f"[{datetime.now()}] FEHLER: Kann Local LLM Model-Info nicht abrufen nach {max_retries} Versuchen"
                    )
                    self.model_name = "unknown"
...
class OpenAILLMProvider(LLMProvider):
    ...
    def __init__(self, api_key: str = None, model: str = None):
        ...
        if not self.api_key:
            raise ValueError(
                "OpenAI API Key nicht gefunden. Setze OPENAI_API_KEY Umgebungsvariable."
            )
        ...
        print(
            f"[{datetime.now()}] OpenAI Provider initialisiert mit Modell: {self.model}"
        )
...
class RagPipeline:
    ...
    def __init__(
        ...
        self.schema_name = schema_name or os.getenv(
            "EMBEDDINGS_SCHEMA", "embeddings_e5_small"
        )
    ...
    def _fetch_model_info(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(
                        f"[{datetime.now()}] WARNUNG: Kann Modell-Info nicht abrufen (Versuch {attempt+1}/{max_retries}): {e}"
                    )
                    time.sleep(retry_delay)
                else:
                    print(
                        f"[{datetime.now()}] FEHLER: Kann Modell-Info nicht abrufen nach {max_retries} Versuchen: {e}"
                    )
    ...
    def _initialize_db(self):
        ...
        try:
            ...
            print(
                f"[{datetime.now()}] WARNUNG: Kann DuckDB im Read-Only nicht öffnen: {e}"
            )
    ...
    def similarity_search(self, user_input: str, k: int = 2) -> List[dict]:
        ...
        try:
            ...
            print(
                f"[{datetime.now()}] Query-Embedding erstellt (Größe: {len(query_embedding)})"
            )
            ...
            print(
                f"[{datetime.now()}] Embedding Dimension: {self.embedding_dim}, Query Embedding Size: {len(query_embedding)}"
            )
            raise
...
"""
    ...
    def query_llm(self, prompt: str, temperature: float = 0.0) -> str:
        ...
        try:
            print(
                f"[{datetime.now()}] Sende Query an LLM-Provider (Modell: {self.llm_provider.get_model_name()})..."
            )
    ...
    def answer_query(
        self, user_input: str, k: int = 2, temperature: float = 0.7
    ) -> dict:
        """
